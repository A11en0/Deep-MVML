...
def br_ce_loss(label, alpha, c, global_step, annealing_step):
    S = torch.sum(alpha, dim=1, keepdim=True)
    E = alpha - 1
    A = torch.sum(label * (torch.digamma(S) - torch.digamma(alpha)), dim=1, keepdim=True)
...
class TMC(nn.Module):
    ...
    def __init__(self, classes, views, classifier_dims, lambda_epochs, device):
        """
        ...
        self.Classifiers = nn.ModuleList([Classifier(classifier_dims[i], self.classes).to(device=device) for i in range(self.views)])
    ...
    def forward(self, X, y, global_step):
        ...
        for v_num in range(len(X)):
            ...
            loss += br_ce_loss(y, alpha[v_num], self.classes, global_step, self.lambda_epochs)
        alpha_a = self.DS_Combin(alpha)
        evidence_a = alpha_a - 1
        loss += br_ce_loss(y, alpha_a, self.classes, global_step, self.lambda_epochs)
